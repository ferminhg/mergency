...
- End-to-end verification (`## Verification`, points 1-2) was confirmed: the stack came up healthy, and a real `pull_request` webhook delivery from a live PR was received and answered with `200`. Points 3-4 (a deliberate CI failure + revert, and a PR budget-status comment) were not exercised in this pass.
