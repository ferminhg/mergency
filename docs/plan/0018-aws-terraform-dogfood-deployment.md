...
- **Region:** the target AWS account enforces a Service Control Policy that restricts operations to `eu-north-1`, not the `eu-west-1` used as the default in Step 2. `variables.tf`'s `aws_region` default was changed to `eu-north-1` to match. Everything else in the plan applied as written once that was corrected.
- **`scripts/github_app_manifest.py` bug:** the manifest generated by this script included `installation` and `installation_repositories` in `default_events`, which GitHub's manifest validator rejects (those events are delivered automatically on install/uninstall, they aren't subscribable). Fixed by removing both from the list.
- **`.env` private key formatting:** `MERGENCY_GITHUB_PRIVATE_KEY` must be wrapped in double quotes in `.env`, with literal `\n` sequences — Docker Compose's `env_file` loader (used by `docker-compose.yml`) only expands `\n` into real newlines for double-quoted values. This wasn't called out explicitly in Step 10 of the original plan.
- End-to-end verification (`## Verification`, points 1-2) was confirmed: the stack came up healthy, and a real `pull_request` webhook delivery from a live PR was received and answered with `200`. Points 3-4 (a deliberate CI failure + revert, and a PR budget-status comment) were not exercised in this pass — they were completed later, tracked as issues [#42](https://github.com/ferminhg/mergency/issues/42) and [#43](https://github.com/ferminhg/mergency/issues/43). See [docs/plan/0019-pr-comment-bot-live-verification.md](0019-pr-comment-bot-live-verification.md): a build-failure + revert was pushed to `main` (2 events recorded for owner `unassigned`, `remaining_pct=0%`), and [PR #48](https://github.com/ferminhg/mergency/pull/48) confirmed Mergency posted the budget-status comment. Getting there also surfaced and fixed real bugs (missing `redis`/`worker` services, missing `issues: write` App permission) — see issue #41's closing comment for the full trail.

See [README.md](../../README.md#deployment--aws-dogfood) for the condensed, copy-pasteable version of these steps.
